...
export default {
  ...
  async mounted () {
    this.userId = localGet('userId')
    let [ [ createdPlaylist, collectedPlaylist ], { signersCount, radiosCount, createdPlaylistCount, collectedPlaylistCount } ] = await Promise.all([getPersonalPlaylists(this.userId), getPersonalOptionsCount()])
